...
- **菱形 = 合并节点**：把别的分支的改动带进本分支的那次合并。点它 = 看本分支上这个文件
  合并前 / 合并后的对比（`git diff <合并提交>^1 <合并提交> -- <文件>`）；
  按住 **Alt 点** = 去看改动源头那个版本自己的改动
- 虚线箭头 = 改动往上走的一跳，**只连相邻两条轨道**（`fb → uat → release → master`
  是一串短箭头，不是从源头横穿全图的长线）；分叉线每条轨道只画一根，画在分支点上
- 浅色虚线轨道 = 这条分支只是继承了这个改动，自己没改过

每个分支的轨道上按时间排着它自己的版本节点和合并节点，所以“这条分支上这个文件到底
经历了什么”是顺着一条竖线从上往下读出来的，不用去追跨分支的线。

**悬停信息框是可以复制文字的**：鼠标移到任意节点 / 箭头 / 轨道上会弹出信息框（原生
`<title>` 提示选不中文字，所以换成了 HTML 浮层），里面的内容可以直接拖选，
鼠标移进框里不会消失，右上角还有一个「复制」按钮把整段信息（含 sha、作者、时间）
一并发到剪贴板。
...
| --- | --- |
| 本地（默认） | 只看 `refs/heads/*` |
| 远程 | 只看 `refs/remotes/*`（`origin/dev` 这种）。想确认“远端仓库上现在是哪一版”就用它 |
| 全部 | 两边都看；本地已有的同名远端镜像（`origin/dev` vs `dev`）只画一根，远端独有的分支会补上 |

固定分支的识别在三种范围下都成立：配置里写 `dev`，看远程时会自动匹配 `origin/dev`。
...
| `gitTree.branchScope` | `local` | 看哪些分支：`local` / `remote` / `all`。面板右上角也能随时切（以面板上的选择为准）。 |
| ~~`gitTree.includeRemotes`~~ | `false` | **已弃用**，改用 `gitTree.branchScope`。它开着等价于 `all`。 |
...
- `test/regression.test.ts` —— 记录下来的"真的算错过"的场景- `test/branchview.test.ts` —— 分支视图那套：合并节点必须是个真合并、且第一父在本分支上；
  传播箭头只连相邻轨道；分支范围的三种取值
